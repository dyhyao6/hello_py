...
class PassengerOverflowProducer:
    def __init__(self, state_file="state/passenger_overflow_new_plan.json"):
        self.state_file = state_file
        self.es = self.get_es_client()
        self.kafka_producer = self.get_kafka_producer()
        self.kafka_topic = os.getenv("KAFKA_TOPIC_PASSENGER_OVERFLOW", "passenger_overflow_new_plan")
            ...
            "query": {
                "bool": {
                    "must": [
                        {
                            "term": {
                                "message.keyword": "passenger overflow new plan"
                            }
                        },
                        {
                            "range": {
                                "time": {
                                    "gte": "2026-04-03T00:00:00.000000+08:00"
                                }
                            }
                        }
                    ]
        ...
        try:
            ...
            for hit in hits:
                ...
                record = {
                    ...
                    "person_num": extra_data_obj.get("person_num", 0),
                    "alert_level": extra_data_obj.get("alert_level", 0)
                }
    ...
    def run(self, interval=5):
        logger.info("Starting Passenger Overflow Producer...")
        logger.info(f"Fetching data from Elasticsearch and sending to Kafka topic: {self.kafka_topic}")
        logger.info(f"Filter: message.keyword = 'passenger overflow new plan'")
        ...
        try:
            while self.running:
                ...
                if new_data:
                    logger.info(
                        f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Found PassengerOverflow {len(new_data)} new records")
...
if __name__ == "__main__":
    producer = PassengerOverflowProducer()
    producer.run()
